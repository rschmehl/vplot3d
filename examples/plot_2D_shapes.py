...
# Shapes
voff = np.array([0, 0, 0])
a1 = Polygon.rotated(PO, file=lib_path / 'data' / 'clark_y_airfoil.dat',
...
a2 = Polygon.rotated(PO, file=lib_path / 'data' / 'kite_V3_airfoil.dat',
                     e1=Px, e2=Py, voff=voff, edgecolor='k', facecolor='k',
                     scale=1, linewidth=2, alpha=0.1, edgecoloralpha=0.8)
ax.annotate3D('TU Delft V3 kite  center airfoil', xyz=0.25*Px+0.15*Py)

voff = np.array([0.5, 0.6, 0])
a3_1 = Polygon.rotated(PO, file=lib_path / 'data' / 'kite_V3_tubeframe.dat',
                       e2=Py, e3=Pz, voff=voff, edgecolor='k', facecolor='k',
                       scale=1e-4, zorder=10, linewidth=10, alpha=0, edgecoloralpha=1)
a3_2 = Polygon.rotated(PO, file=lib_path / 'data' / 'kite_V3_planform.dat',
                       e2=Py, e3=Pz, voff=voff, edgecolor='k', facecolor='w',
                       scale=1e-4, zorder=0, linewidth=2, alpha=0.8, edgecoloralpha=0.8)
